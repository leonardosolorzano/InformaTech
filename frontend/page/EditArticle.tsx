...
import { fetchArticle, updateArticle, uploadArticleImage, imageUrl } from "../src/api";
...
export default function EditArticle() {
    ...
    fetchArticle(Number(id), token)
      .then((article) => {
        ...
        setForm({
          ...
          fecha_publication: article.fecha_publication,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setFormError("");
    try {
      await updateArticle(Number(id), form, token);
      navigate("/dashboard");
    } catch (err) {
  ...
  if (!user) {
    return (
      <div className="text-center py-20 text-gray-500">
        <p className="text-lg">Inicia sesión para editar artículos.</p>
      </div>
    );
  }

  if (loading) {
    return <p className="text-gray-400 text-center py-20">Cargando artículo...</p>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Modifica el contenido de tu artículo</p>
      </div>
